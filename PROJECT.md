...
Total = I1 * T(I1) + (G1+G2+G3+G4) * C(G1-G4) + (G1+G2+G3+G4) * C(G5) + I2 * T(I2),
Где C - contribution, вклад студента в проект


### G1 Старт проекта 12 баллов
...
После merge нужно создать группу в телеграмме с именем WebAppDev-2025-{projectId} и добавить туда всех участников проекта и преподавателя.

Напишите в чате команды о предлагаемой теме, тегните преподавателя, он даст апрув или рекомендации по именованию проекта.


|  | До 17.09 | До 24.10 | После 24.10|
|--|----------|----------|------------|
|T| 1        | 0.8      | 0.6        |
...
|--|--------|------------------|------------------|
|G1.0| Презентация (слайды/видео/landing page) проекта дает четкое понимание о проекте | 2             | 3                |
|G1.1| Схематичные макеты подробно описывают пользовательский flow | 2             | 3                |
|G1.2| Схема содержит подробную информацию о потоке данных в приложении | 2             | 3                |
|G1.3| Произведено краткосрочное планирование. есть план работ на ближайшие 2 недели, задачи заведены, назначены ответственные, у задач есть четкое описание, DOD | 2             | 3                |
...
### G5 Финальные результаты 22 балла
|  | До 14.01  | до 19.01| после 19.01 |
|--|----------|----------|----------|
|T| 1        | 0.8      | 0.6
...
|R | Оценка | Минимальный балл | Максимальный балл |
|--|--------|------------------|------------------|
|G5.0| К проекту подключена аналитика  | 0.6            | 1            |
|G5.1| Все техничесские требования к серверу выполнены   | 1.2            | 2           |
|G5.2| Использованы WebApi из списка требований | 1.2 | 2 |
|G5.3| 100% фичей реализовано | 10           | 16 |
|G5.4| Презентация результата | 0.6           | 1 |

### I2 Индивидуальная защита 24 балла

|  | До 19.01 | после  19.01|
|--|----------|----------|
|T | 1        | 0.6      |
...
|R | Оценка | Минимальный балл | Максимальный балл |
|--|--------|------------------|------------------|
|I2.0| Решение задачи по проекту | 3.6            | 6          |
|I2.1| [Теоретический вопрос](./thereticalQuestions.mjs) | 8.4        | 18|

В рамках этой контрольной считается коэффициент вклада в проект C(G5)

I2.0 / I2.1 можно будет не сдавать, если
1) ваш контрибьюшн в проект  (C(G1-G4) + C(G5)) / 2 >= 0.9
2) за I1 * T(I1) вы получили >=15 баллов
3) вас устраивает набранный балл.

I2.0 / I2.1 засчитывается автоматом студентам с набранным количеством баллов больше либо равным 73.
...
<details>
<summary>Можно ли сдавать контрольные точки не по порядку?
</summary>
