...
- к приложению должна быть подключена система баг трекинга, например sentry/ bugsnag и другие.
...
Внимание, контрольные точки существуют чтобы держать разработку в ритме, 
подразумевается, что по проекту делается работа помимио закрытия контрольных точек

### 1 Старт проекта 15 баллов
...
Где, GX Коэффициент срока:
|  | До 26.09 | До 03.10 | после 03.10|
|--|----------|----------|------------|
|GX| 1        | 0.8      | 0.6        |

- G0 - презентация проекта дает четкое понимание о проекте 3 баллов.
- G1 - схематичный макеты подробно описывают пользовательский flow - 5 баллов
- G2 - cхема содержит подробную информацию о потоке данных в приложении. 5 балла
- G3 - произведено краткосрочное планирование. есть план работ на ближайшие 2 недели, задачи заведены, назначены ответственные, у задач есть четкое описание, DOD. 2 балла.
...
Используйте excalidraw с совместным доступом или miro и подобные инструменты.
...
Планирование можно делать в инструментах по типу jira/trello/miro/wrike/ etc.
...
### 2 Подробное планирование 8 баллов
...
- G1 - Есть подробные макеты как выглядит приложение - 4 балла
- G2 - Есть описание всей архитектуры приложения (Схема). Какие модули, какие технологии, какие API используются. Как модули связаны друг с другом, какие протоколы используются. 4

### 3 Автоматизация, CI 12 баллов
...
- G0 - в репозитории настроены правила контрибьюта (например, нельзя коммитить в мастер напрямую, есть шаблоны для PR) 1 балл
- G1 - в проекте настроен запуск unit тестов - 2 балла
- G2 - в проекте настроен запуск e2e тестов - 3 балла
- G3 - в проекте настроен test coverage для unit тестов - 1 балл
- G4 - CI проекта не разрешает merge PR если не прошли unit тесты - 1 балл
- G5 - CI проекта не разрешает merge PR если тест coverage ниже 80% - 1 балл
- G6 - CI проекта не разрешает merge PR если e2e тесты не прошли - 1 балл.
- G7 - в проекте есть стандартизация кода, применяется eslint/prettier - 1 балл
- G8 - СI проекта не разрешает merge PR если не прошли проверки eslint/prettier - 1 балл
...
### Индивидуальная защита 20 баллов

28.10 и 4.11

Ожидается что к этому моменту, каждый участник создаст PR/ пройдет ревью реализовав какую-то функциональность. Это должны быть полноценные PR ы с тестами. Объем контрибьюшена будет оценен.
На защите буду вопросы по тому коду, который есть в проекте.
...
- G0 - должны
- G1 - реализовано 40% запланированных фичей
- G2 - все фичи покрыты тестами
- G3 - все мерджи в мастер должны автоматически деплоиться
