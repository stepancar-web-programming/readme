...
|  | Оценка | Минимальный балл | Макимальный балл |
|--|--------|------------------|------------------|
|G0| Презентация проекта дает четкое понимание о проекте | 3             | 5                |
|G1| Схематичные макеты подробно описывают пользовательский flow | 3             | 5                |
|G2| Схема содержит подробную информацию о потоке данных в приложении | 3             | 5                |
|G3| Произведено краткосрочное планирование. есть план работ на ближайшие 2 недели, задачи заведены, назначены ответственные, у задач есть четкое описание, DOD | 3             | 5                |
...
|  | Оценка | Минимальный балл | Макимальный балл |
|--|--------|------------------|------------------|
|G0| в репозитории настроены правила контрибьюта (например, нельзя коммитить в мастер напрямую, есть шаблоны для PR, рекомендуется работать по [github flow](https://docs.github.com/ru/get-started/using-github/github-flow))     | 0.6              | 1                |
|G1| в проекте настроен запуск unit тестов | 1.2             | 2                |
|G2| в проекте настроен запуск e2e тестов | 1.8              | 3                |
|G3| в проекте настроен test coverage для unit тестов | 0.6              | 1                |
|G4| CI проекта не разрешает merge PR если не прошли unit тесты | 0.6              | 1                |
|G5| CI проекта не разрешает merge PR если тест coverage ниже 80% | 0.6              | 1                |
|G6| CI проекта не разрешает merge PR если e2e тесты не прошли | 0.6              | 1                |
|G7| в проекте есть стандартизация кода, применяется eslint/prettier | 0.6              | 1                |
|G8| СI проекта не разрешает merge PR если не прошли проверки eslint/prettier | 0.6              | 1                |
...
|  | Оценка | Минимальный балл | Макимальный балл |
|--|--------|------------------|------------------|
|G1| 40% фичей реализовано | 1.2             | 2            |
|G2| все фичи покрыты тестами | 1.2          | 2            | 
|G3| все мерджи в мастер должны автоматически деплоиться | 1.2              | 2                | 
